...
def get_kvps(result, threshold=0.85, n_pages=False):
    kvps = [
        kvp
        for kvp in result.key_value_pairs
        if kvp.confidence >= threshold and kvp.value
    ]
    ...
    to_keep = (
        pd.concat([kvps["key"].str.lower(), kvps["value"].str.lower()], axis=1)
        .drop_duplicates()
        .index
    )
    ...
    for pattern in BAD_PATTERNS:
        kvps = kvps[~kvps["key"].str.strip().str.contains(pattern, case=False)]
    # Drop irrelevant values
    kvps = kvps[~kvps["value"].str.strip().str.contains(r"^N\.A(\.)?$", case=False)]
...
def extract(handler):
    ...
    result = yield gramex.service.threadpool.submit(
        ocr, path, endpoint=secrets["AZURE_OCR_URL"], api_key=secrets["AZURE_OCR_KEY"]
    )
    doctype = yield gramex.service.threadpool.submit(defects.get_doctype, result)
    annex = yield gramex.service.threadpool.submit(
        defects.get_annexures_index, result.tables
    )
    bad_quality = yield gramex.service.threadpool.submit(
        defects.bad_quality_pages, result
    )

    return {
        "url": path,
        "kvps": get_kvps(result),
        "doctype": doctype,
        "annex": annex,
...
if __name__ == "__main__":
    ...
    for file in tqdm(glob.glob("data/files/*.pdf")):
        result = ocr(
            file, endpoint=secrets_.AZURE_OCR_URL, api_key=secrets_.AZURE_OCR_KEY
        )
        kvps = []
        for kvp in result.key_value_pairs:
            if kvp.confidence >= 0.9:
                kvps.append(
                    (kvp.key.content, kvp.value.content if kvp.value else kvp.value)
                )
